        ...
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                {!! Form::model($service, ['route' => [config('laraadmin.adminRoute') . '.services.update', $service->id ], 'method'=>'PUT', 'id' => 'service-edit-form']) !!}
					...
					@la_input($module, 'import')
					@la_input($module, 'export')
					@la_input($module, 'type')
                    --}}
                    <br>
                    <div class="form-group">
                        {!! Form::submit( 'Update', ['class'=>'btn btn-success']) !!} <a href="{{ url(config('laraadmin.adminRoute') . '/services') }}" class="btn btn-default pull-right">Cancel</a>
                    </div>
                {!! Form::close() !!}
            </div>
        </div>
